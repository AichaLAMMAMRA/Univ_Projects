...
void affiche_matrix(double **A,int m){ 
    ...
    printf ("     _________  matrice _________\n" );
    for(int i=0;i<m-1;i++) {
...
double* dev_vecteur_scal(double *vector,double scal,int n, double *result){ 
    ...
    prod_vector_scal(vector , 1/scal, n, result);   
}
...
double* prod_mtrix_vector(double **matrix,double *vector,int n){
    ...
    #pragma omp parallel for default (none) shared (matrix, vector, result,n) private (i,j)
