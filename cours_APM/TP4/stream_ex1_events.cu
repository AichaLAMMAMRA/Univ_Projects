...
int main(int argc, char **argv)
{
    ...
    cudaEvent_t start[NSTREAMS];
    cudaEvent_t stop[NSTREAMS];
    ...
    {
      cudaEventCreate(&(start[i]), i);
      cudaEventCreate(&(stop[i]), i);
    }
    ...
    for (int i = 0; i< NSTREAMS; ++i)
    {
      ...
      printf("Elapsed Time Kernel %d = %f\n", i, elapsedTime);

    }
	cudaEventElapsedTime(&elapsedTime, start[0], stop[NSTREAMS-1]);
